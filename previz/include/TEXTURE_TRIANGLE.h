#ifndef TEXTURE_TRIANGLES_H
#define TEXTURE_TRIANGLES_H
...
class TextureTriangle : public Primitive {
private:
    POINT3 vert1;
    POINT3 vert2;
    POINT3 vert3;
...
public:
    TextureTriangle(const POINT3& V1, const POINT3& V2, const POINT3& V3, const double& phong, shared_ptr<Material> material, float *textureSource, int tXRes, VEC2 t1, VEC2 t2, VEC2 t3) 
    ...
    bool hit(const Ray& r, double t0, double t1, hit_record& rec) const override {
        ...
        if (t > e)
        {
            if(t < t1 && t > t0) {
                ...
                int idealRes = 10000;
                double expansion = idealRes / (float)textXRes;
